...
import { Search, Filter, Download, AlertTriangle, CheckCircle, XCircle, Clock, User, Settings, Database, Eye, Activity } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{auditStats.total.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              +{auditStats.today} hoy
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{auditStats.success.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              {Math.round((auditStats.success / auditStats.total) * 100)}% del total
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{auditStats.failed.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              {Math.round((auditStats.failed / auditStats.total) * 100)}% del total
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{auditStats.bySeverity.high}</div>
            <p className="text-xs text-muted-foreground">
      ...
      <Tabs defaultValue="logs" className="space-y-4">
        ...
        <TabsContent value="logs" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Historial completo de todas las actividades del sistema
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Filtros */}
                        ...
                        <TableCell>
                          <Badge className={getSeverityBadge(log.severity)}>
                            {log.severity === 'info' ? 'Info' : 
                             ...
                             log.severity === 'medium' ? 'Media' : 'Alta'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                    ...
                    <div key={category} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 bg-gray-200 rounded-full h-2">
                          <div className="bg-blue-600 h-2 rounded-full" style={{width: `${(count / auditStats.total) * 100}%`}}></div>
                        </div>
                    ...
                    <div key={severity} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <div className={`w-3 h-3 rounded-full ${
                          ...
                          severity === 'medium' ? 'bg-orange-500' : 'bg-red-500'
                        }`}></div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 bg-gray-200 rounded-full h-2">
                          <div className={`h-2 rounded-full ${
                            ...
                            severity === 'warning' ? 'bg-yellow-500' :
                            severity === 'medium' ? 'bg-orange-500' : 'bg-red-500'
                          }`} style={{width: `${(count / auditStats.total) * 100}%`}}></div>
                        </div>
                        <span className="text-sm font-medium">{count.toLocaleString()}</span>
                      </div>
                    </div>
        ...
        <TabsContent value="alerts" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {auditLogs.filter(log => log.severity === 'high' || log.status === 'failed').map((log) => (
