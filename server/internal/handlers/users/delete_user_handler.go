...
func RestoreUserHandler(c fiber.Ctx) error {
	id := c.Params("id")

	if err := services.RestoreUser(id); err != nil {
		if err.Error() == "usuario no encontrado o no está eliminado" || err.Error() == "ID inválido" {
			return c.Status(fiber.StatusNotFound).
				JSON(dto.ErrorResponse{Error: err.Error()})
		}
		logger.Log.Error("Error restaurando usuario:", err)
		return c.Status(fiber.StatusInternalServerError).
			JSON(dto.ErrorResponse{Error: "Error interno del servidor"})
	}
