...
func CreateUserApplicationRoleHandler(c fiber.Ctx) error {
	var req dto.CreateUserApplicationRoleRequest
	...
	result, err := services.CreateUserApplicationRole(req, user.ID)
	if err != nil {
		if err.Error() == "el usuario ya tiene este rol asignado en esta aplicación" {
		...
		logger.Log.Error("Error creando asignación de rol:", err)
