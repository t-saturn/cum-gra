...
const CardStat = ({ stat }: { stat: CardStatType }) => {
  return (
    <Card key={stat.title} className="border-border bg-card/50 backdrop-blur-sm hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
      ...
      <CardContent>
        ...
        {stat.trend && stat.trendIcon && (
          <div className="flex items-center text-sm">
            ...
            <span className="text-muted-foreground ml-1">{stat.trendText}</span>
          </div>
        )}
      </CardContent>
    </Card>
