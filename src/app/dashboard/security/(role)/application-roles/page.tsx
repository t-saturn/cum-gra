...
import { Search, Filter, Plus, Edit, Trash2, Users, ChevronDown, ChevronRight, Eye, Copy } from 'lucide-react';
...
const applicationRoles = [
  ...
  },
  {
    ...
    applicationName: 'CRM System',
    level: 'user',
    permissions: ['crm.leads.read', 'crm.leads.write', 'crm.opportunities.read', 'crm.opportunities.write', 'crm.contacts.read', 'crm.contacts.write', 'crm.reports.read'],
    userCount: 23,
    isActive: true,
  ...
  },
  {
    ...
    level: 'viewer',
    permissions: ['analytics.dashboards.read', 'analytics.reports.read', 'analytics.charts.read', 'analytics.data.read'],
    userCount: 45,
    isActive: true,
...
export default function ApplicationRolesManagement() {
  ...
  const filteredRoles = applicationRoles.filter((role) => {
    ...
    const matchesApplication = applicationFilter === 'all' || role.applicationName === applicationFilter;
  ...
  const togglePermissionExpansion = (roleId: string) => {
    setExpandedPermissions((prev) => (prev.includes(roleId) ? prev.filter((id) => id !== roleId) : [...prev, roleId]));
  };
  ...
  const groupPermissionsByCategory = (permissions: string[], applicationName: string): Record<string, Permission[]> => {
    const appPermissions: Permission[] = availablePermissions[applicationName] || [];
        ...
        <TabsContent value="roles" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Administra los roles específicos para cada aplicación del sistema</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Filtros y acciones */}
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
                <div className="flex flex-1 items-center space-x-2">
                  ...
                  <Input placeholder="Buscar roles..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-sm" />
                </div>
                  ...
                  <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                    ...
                    <DialogContent className="max-w-2xl">
                      <DialogHeader>
                        ...
                        <DialogDescription>Define un nuevo rol con permisos específicos para una aplicación</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label htmlFor="name">Nombre del Rol</Label>
                            <Input id="name" value={newRole.name} onChange={(e) => setNewRole({ ...newRole, name: e.target.value })} placeholder="Ej: CRM Manager" />
                          </div>
                          <div>
                        ...
                        <div>
                          ...
                          <Select value={newRole.applicationId} onValueChange={(value) => setNewRole({ ...newRole, applicationId: value })}>
                            <SelectTrigger>
                              <SelectValue placeholder="Selecciona una aplicación" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="app_001">CRM System</SelectItem>
                              <SelectItem value="app_002">ERP System</SelectItem>
                              <SelectItem value="app_003">Analytics Platform</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                        ...
                        {newRole.applicationId && (
                          <div>
                            ...
                            <div className="border rounded-md p-4 max-h-60 overflow-y-auto">
                              {(() => {
                                const system = newRole.applicationId === 'app_001' ? 'CRM System' : newRole.applicationId === 'app_002' ? 'ERP System' : 'Analytics Platform';
                                const permissions: Permission[] = availablePermissions[system] || [];

                                return Object.entries(
                                  permissions.reduce((acc: Record<string, Permission[]>, perm: Permission) => {
                                    if (!acc[perm.category]) acc[perm.category] = [];
                                    acc[perm.category].push(perm);
                                    return acc;
                                  }, {}),
                                ).map(([category, perms]: [string, Permission[]]) => (
                                        ...
                                        <div key={perm.id} className="flex items-center space-x-2">
                                          <Checkbox
                                            ...
                                            onCheckedChange={(checked) => {
                                              setNewRole({
                                                ...newRole,
                                                permissions: checked ? [...newRole.permissions, perm.id] : newRole.permissions.filter((p) => p !== perm.id),
                                              });
                                            }}
                                          />
                        ...
                        <TableCell>
                          <Collapsible>
                            <CollapsibleTrigger asChild>
                              <Button variant="ghost" size="sm" onClick={() => togglePermissionExpansion(role.id)}>
                                {expandedPermissions.includes(role.id) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                                {role.permissions.length} permisos
                              </Button>
                            </CollapsibleTrigger>
                            <CollapsibleContent className="mt-2">
                              <div className="space-y-2">
                                {Object.entries(groupPermissionsByCategory(role.permissions, role.applicationName)).map(([category, perms]) => (
                                  <div key={category}>
        ...
        <TabsContent value="permissions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Vista consolidada de todos los permisos por aplicación y rol</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                {Object.entries(availablePermissions).map(([appName, permissions]) => (
                  <div key={appName} className="space-y-4">
                    <h3 className="text-lg font-semibold">{appName}</h3>
                    <div className="grid gap-4">
                      {Object.entries(
                        permissions.reduce((acc: Record<string, { id: string; name: string; category: string }[]>, perm) => {
                          if (!acc[perm.category]) acc[perm.category] = [];
                          acc[perm.category].push(perm);
                          return acc;
                        }, {}),
                              ...
                              {perms.map((perm: { id: string; name: string }) => (
                                <div key={perm.id} className="flex items-center justify-between p-2 border rounded">
                                  ...
                                  <div className="flex space-x-2">
                                    {applicationRoles
                                      .filter((role) => role.applicationName === appName)
                                      .map((role) => (
                                        <Badge key={role.id} variant={role.permissions.includes(perm.id) ? 'default' : 'outline'} className="text-xs">
                                          {role.name}
                                        </Badge>
                                      ))}
                                  </div>
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-3 h-3 rounded-full ${
                            level === 'admin' ? 'bg-red-500' : level === 'manager' ? 'bg-orange-500' : level === 'user' ? 'bg-blue-500' : 'bg-gray-500'
                          }`}
                        ></div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {Object.entries(roleStats.byApplication).map(([app, count]) => (
