...
import { fn_bulk_create_positions, BulkUploadResponse } from '@/actions/positions/fn_bulk_create_positions';
import { fn_download_positions_template } from '@/actions/positions/fn_download_positions_template';
...
export default function BulkUploadPositionsModal({ open, onOpenChange, onSuccess }: BulkUploadModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      setDownloading(true);
      const blob = await fn_download_positions_template();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `plantilla_posiciones_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fn_bulk_create_positions(formData);
      setResult(response);

      if (response.created > 0) {
        toast.success(`${response.created} posiciones creadas correctamente`);
        onSuccess();
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="bg-card/80 backdrop-blur-xl border-border sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Carga Masiva de Posiciones</DialogTitle>
          <DialogDescription>
            Sube un archivo Excel con las posiciones a registrar
          </DialogDescription>
        </DialogHeader>
          ...
          {!result && (
            <div className="bg-muted/50 p-4 rounded-lg">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>• Columnas: name, code, level, description, is_active, cod_car_sgd</li>
        ...
        <DialogFooter>
          ...
          {!result && (
            ...
            >
              {loading ? 'Procesando...' : 'Cargar Posiciones'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
