...
func BulkAssignRolesToUserHandler(c fiber.Ctx) error {
	var req dto.BulkAssignRolesToUserRequest
	...
	result, err := services.BulkAssignRolesToUser(req, user.ID)
	if err != nil {
		...
		logger.Log.Error("Error asignando roles masivamente a usuario:", err)
...
func BulkAssignRoleToUsersHandler(c fiber.Ctx) error {
	var req dto.BulkAssignRoleToUsersRequest
	if err := c.Bind().Body(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).
			JSON(dto.ErrorResponse{Error: "Datos inválidos"})
	}
	...
	result, err := services.BulkAssignRoleToUsers(req, user.ID)
	if err != nil {
		if err.Error() == "aplicación no encontrada" ||
		   err.Error() == "rol no encontrado" ||
		   err.Error() == "el rol no pertenece a la aplicación especificada" ||
		   err.Error() == "application_id inválido" ||
		   err.Error() == "application_role_id inválido" {
			return c.Status(fiber.StatusBadRequest).
				JSON(dto.ErrorResponse{Error: err.Error()})
		}
		logger.Log.Error("Error asignando rol masivamente a usuarios:", err)
		return c.Status(fiber.StatusInternalServerError).
			JSON(dto.ErrorResponse{Error: "Error interno del servidor"})
	}
