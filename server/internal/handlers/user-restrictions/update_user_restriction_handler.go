...
func UpdateUserModuleRestrictionHandler(c fiber.Ctx) error {
	id := c.Params("id")

	var req dto.UpdateUserModuleRestrictionRequest
	...
	result, err := services.UpdateUserModuleRestriction(id, req, user.ID)
	if err != nil {
		if err.Error() == "restricción no encontrada" || err.Error() == "ID inválido" {
			return c.Status(fiber.StatusNotFound).
				JSON(dto.ErrorResponse{Error: err.Error()})
		}
		if err.Error() == "formato de fecha inválido para expires_at" {
			return c.Status(fiber.StatusBadRequest).
				JSON(dto.ErrorResponse{Error: err.Error()})
		}
		logger.Log.Error("Error actualizando restricción:", err)
