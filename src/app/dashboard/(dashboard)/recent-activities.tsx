...
const RecentActivities = () => {
  return (
    <CardCustom title="Actividad Reciente" description="Últimas acciones realizadas en el sistema" order="col" Icon={ClipboardList}>
      {recentActivities.map((activity) => (
        <div key={activity.id} className="flex items-start gap-4 p-3 rounded-lg bg-accent/50 transition-colors">
          <div
            className={`p-2 rounded-full ${
              activity.status === 'success' ? 'bg-chart-4/20 text-chart-4' : activity.status === 'warning' ? 'bg-chart-5/20 text-chart-5' : 'bg-chart-2/20 text-chart-2'
            }`}
          >
