...
import { fn_bulk_create_organic_units, BulkUploadResponse } from '@/actions/units/fn_bulk_create_organic_units';
import { fn_download_organic_units_template } from '@/actions/units/fn_download_organic_units_template';
...
export default function BulkUploadOrganicUnitsModal({ open, onOpenChange, onSuccess }: BulkUploadModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      setDownloading(true);
      const blob = await fn_download_organic_units_template();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `plantilla_unidades_organicas_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fn_bulk_create_organic_units(formData);
      setResult(response);

      if (response.created > 0) {
        toast.success(`${response.created} unidades creadas correctamente`);
        onSuccess();
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="bg-card/80 backdrop-blur-xl border-border sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Carga Masiva de Unidades Orgánicas</DialogTitle>
          <DialogDescription>
            Sube un archivo Excel con las unidades a registrar
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Incluye hoja de referencia con unidades existentes</p>
              </div>
            </div>
          ...
          {!result && (
            <div className="bg-muted/50 p-4 rounded-lg">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>• Columnas: name, acronym, brand, description, parent_id, is_active, cod_dep_sgd</li>
        ...
        <DialogFooter>
          ...
          {!result && (
            ...
            >
              {loading ? 'Procesando...' : 'Cargar Unidades'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
