...
import { Search, Filter, Download, Shield, Monitor, Smartphone, Tablet, MapPin, Clock, AlertTriangle, X, Eye, Ban } from 'lucide-react'
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {Math.round((sessionStats.active / sessionStats.total) * 100)}% del total
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Sesiones en Riesgo</CardTitle>
            <AlertTriangle className="h-4 w-4 text-red-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{sessionStats.byRisk.high}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Dispositivos Móviles</CardTitle>
            <Smartphone className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{sessionStats.byDevice.mobile + sessionStats.byDevice.tablet}</div>
            <p className="text-xs text-muted-foreground">
              {Math.round(((sessionStats.byDevice.mobile + sessionStats.byDevice.tablet) / sessionStats.total) * 100)}% del total
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="sessions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Monitorea y controla todas las sesiones activas en tiempo real
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Filtros */}
                        ...
                        <TableCell>
                          <Badge className={getStatusBadge(session.status).className}>
                            {session.status === 'active' ? 'Activo' : 
                             session.status === 'idle' ? 'Inactivo' : 'Advertencia'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Badge className={getRiskBadge(session.riskLevel)}>
                            {session.riskLevel === 'low' ? 'Bajo' : 
                             session.riskLevel === 'medium' ? 'Medio' : 'Alto'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ¿Estás seguro de que quieres terminar la sesión de {session.userName}? 
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-blue-600 h-2 rounded-full" style={{width: `${(sessionStats.byDevice.desktop / sessionStats.total) * 100}%`}}></div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-green-600 h-2 rounded-full" style={{width: `${(sessionStats.byDevice.mobile / sessionStats.total) * 100}%`}}></div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-purple-600 h-2 rounded-full" style={{width: `${(sessionStats.byDevice.tablet / sessionStats.total) * 100}%`}}></div>
                      </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-green-500 h-2 rounded-full" style={{width: `${(sessionStats.byRisk.low / sessionStats.total) * 100}%`}}></div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-yellow-500 h-2 rounded-full" style={{width: `${(sessionStats.byRisk.medium / sessionStats.total) * 100}%`}}></div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-24 bg-gray-200 rounded-full h-2">
                        <div className="bg-red-500 h-2 rounded-full" style={{width: `${(sessionStats.byRisk.high / sessionStats.total) * 100}%`}}></div>
                      </div>
        ...
        <TabsContent value="security" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sesiones que requieren atención inmediata
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {activeSessions.filter(s => s.riskLevel === 'high' || s.status === 'warning').map((session) => (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {session.riskLevel === 'high' ? 'Sesión de alto riesgo' : 'Actividad sospechosa detectada'}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
